// Generated at 9/2/2026 11:13:10 PM
...
native_string(value_or_template, ...values):
string_is_real(str):
string_to_real(str, ?default_value):
//}
...
string_sha1(str):
string_sha1_utf8(str):
string_md5(str):
string_md5_utf8(str):
string_lpad(str, char, len):
string_rpad(str, char, len):
string_ltrim(str):
string_rtrim(str):
string_split_list(str, delim, ?list):
string_save(str, path):
string_load(path):
//}
...
string_width_ext(string:string, sep, w):
string_height_ext(string:string, sep, w):
//}
...
object_has_event(ind, event_type, event_number):
//}
//{ Motion planning
:mp_linear_step(x, y, speed, checkall)
:mp_potential_step(x, y, speed, checkall)
:mp_linear_step_object(x, y, speed, object)
:mp_potential_step_object(x, y, speed, object)
//}
//{ Raw audio
...
audio_sound_length_nonsync(soundid):
audio_get_type(soundid):
audio_sound_gain(soundid,volume,time):
...
audio_sound_get_gain(soundid):
audio_sound_get_pitch(soundid):
audio_get_name(soundid):
audio_sound_set_track_position(soundid, time)
audio_sound_get_track_position_nonsync(soundid):
//}
//{
matrix_view = 0
matrix_projection = 1
...
:instance_nearest_nonself(x, y, obj):
...
player_is_local_nonsync(player):
player_find_local_nonsync(local_index=0):
dp_player_is_viewed_nonsync(player):
...
view_xview_nonsync[player]*
view_yview_nonsync[player]*
