...
Status as of 2026-09-06, from `git log`. Phases are the §5 subsections; each was done on `main` in one commit and verified with `pnpm test` before committing. Later phases build on the committed state of earlier ones, so read this table before assigning work.
...
2. The dump emits no argument type hints. *(Corrected in the §5.5 row of the Progress table: the 100.034 dump types 341 of 984 functions; the reference adds 252 types on 138.)* The 126 typed signatures in the reference `api.gml` were added by hand by the docs author. The generator merges them by name, so a renamed argument (`obj_or_array` → `obj` in `collision_*`) loses its hint; those hints must be re-attached by position, not by argument name.
...
16. ~~Whether to ship object instance-variable completions for all ~560 objects (from `docs/objects/*.html`, replacing the retired `fields.gml`).~~ **Resolved**: shipped, but not the way this item assumed. `docs/objects/*.html` is six pages, not ~560, so it annotates rather than replaces the dump; the retired `fields.gml` is the only source of breadth and is vendored under its own date. The merged table is 1,493 fields over 489 of 564 objects; the 75 objects the dump never listed are each either newer than the build behind it or skipped by it, and rather than guess at them the extension offers 73 of them nothing at all (`CustomChest` and `CustomPickup` are the two covered by the hand-written `Custom*` table). `UberCont.opt`, `.custom` and `.customMode` from item 4 are added by hand in `api/fields-overrides.gml` with their changelog citations; `UberCont.tips` needed no hand entry, because the vendored `UberCont` page already declares it as `TipsStruct`.
