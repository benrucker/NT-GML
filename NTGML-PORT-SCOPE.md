...
**Status: done, pending commit** (see Progress for deviations). Both files are generated by `tools/generate-grammar.ts` from the committed tables; `package.json` already contributed the paths and scope names below.
